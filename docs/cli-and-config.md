...
命令入口是 `ci-templates`（`python3 -m ci_templates` 等价）。配置来自 `CI_PROJECT_CONFIG_JSON`，或 `--config` / `CI_PROJECT_CONFIG` / 默认 `ci-pipeline.json`。
...
| `build` | `--service`、`--tag`（默认 `dev`）、`--artifact-manifest`、`--preserve-previous` | BuildKit 构建并推送 |
| `prewarm` | | 把 `base_images` 预热到 Harbor |
| `promote-candidate` | `--service`、`--tag` | 用 buildx imagetools 把候选 tag 提升为 `:dev` |
...
| `smoke` | `--repo` | 有 `KUBECONFIG` 时跑集群冒烟，否则跑 `smoke_command` |
...
`smoke`：存在 `KUBECONFIG` 时对 `APPLICATION_NAMESPACE`（默认 `knowledge-core-dev`）做内置 readiness 与公开 document 读路径检查；否则执行 `smoke_command`。
...
| `kustomize_name` | 否 | `knowledge-core-<name>` | Argo Application 名为 `{kustomize_name}-dev` |
...
| 变量 | 谁读 | 作用 |
| --- | --- | --- |
| `CI_PROJECT_CONFIG_JSON` | 配置加载 | 内联 JSON，优先于文件 |
...
| `APPLICATION_NAMESPACE` | smoke | 默认 `knowledge-core-dev` |
