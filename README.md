...
候选 tag 按源 commit 命名且在 Harbor 中设为不可变；工作流重试会通过 `--reuse-existing` 复用已存在的候选 manifest。`restore-candidate` 先核对标签当前 digest：目标 digest 已存在时视为成功，冲突或写入结果不确定时读回验证，绝不覆盖指向其他 digest 的标签。`cleanup-candidate` 只删除仍属于当前最新 attempt 的候选 tag；过期 attempt 或 GitHub run 查询失败时跳过删除，避免重跑竞态。
