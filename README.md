...
| 要理解 ARC runner 池、改容量或给新仓库选 `runs-on` | [ARC 实现](docs/arc.md) |
...
`ENTRYPOINT` 是 `ci-templates`；ARC runner 镜像直接提供同名 CLI，workflow 不再依赖宿主机 Docker socket。workflow 里典型调用：

```text
ci-templates <command> --config .ci/pipeline.yaml
```
